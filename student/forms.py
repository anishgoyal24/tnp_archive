...
class student_profile_form(forms.ModelForm):
    ...
    address = forms.CharField(label="Current Address",required = True, widget = forms.Textarea(attrs = {'class':'materialize-textarea'}))
    ...
    marks_12 = forms.DecimalField(label="XII Percentage (Top 5 Subjects)",max_digits=5,decimal_places=2,required = True,widget = forms.TextInput(attrs = {'class':'validate'}))
    marks_11 = forms.DecimalField(label="X Percentage (Evaluated as CGPA * 9.5)",max_digits=5,decimal_places=2,required = True,widget = forms.TextInput(attrs = {'class':'validate'}))
